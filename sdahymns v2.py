...
def open_in_presenters_view(file_path):
    try:
        ...
        # Get the window handle of the main root app
        root_hwnd = win32gui.GetForegroundWindow()

        # Start the slideshow in Presenter View
        slideshow_settings = presentation.SlideShowSettings
        slideshow_settings.Run()

        # Bring the main root app to the background without minimizing
        win32gui.SetWindowPos(root_hwnd, win32con.HWND_BOTTOM, 0, 0, 0, 0, win32con.SWP_NOSIZE | win32con.SWP_NOMOVE)
